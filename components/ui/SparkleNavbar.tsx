"use client";
// Source: lightswind.com SparkleNavbar (provided verbatim by user).
// Two minimal additions for Next.js integration, both backward-compatible:
//   • `defaultActiveIndex` — initial active item (e.g. derived from pathname)
//   • `onItemSelect`       — fires when an item is clicked, used to navigate
// Visual + animation behaviour is identical to the original snippet.
...
interface SparkleNavbarProps {
  ...
  /** Fires when a non-active item is clicked. Use to wire up navigation. */
...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
  ...
  const handleClick = (index: number) => {
    ...
    // Notify parent — used to wire navigation. Fires once per click, before
    // animation starts so the route swap can happen during the streak.
    onItemSelect?.(index);
